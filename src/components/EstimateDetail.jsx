...
export default function EstimateDetail({ id, onBack }) {
  ...
  // 金額計算
  const subtotal   = data.items.items.reduce((sum, x) => sum + x.amount, 0)
    ...
    <div>
      ...
      >
        ...
        <div style={{ margin:'48px 0', textAlign:'center' }}>
          ...
          <div style={{ fontSize:40, fontWeight:'bold', color:'#1a4', letterSpacing:2 }}>
      ...
      >
        ...
        <table style={{ width:'100%', borderCollapse:'collapse', fontSize:14, marginBottom:24 }}>
          <thead>
            <tr>
              {['内容','別紙添付（有・無）','仕様','数量','単位','単価（円）','金額（円）'].map((h, i) => (
                <th
                  key={i}
                  style={{
                    border:'1px solid #000',
            ...
            {detailRows.map((row, i) => (
              <tr key={i}>
                ...
                <td style={{
                  ...
                  padding:6,
                  textAlign:'right',
                  fontWeight: i >= detailRows.length - 6 ? 'bold' : 'normal',
                  background: i >= detailRows.length - 6 ? '#f0f0f0' : undefined
                }}>
                  {row.amount !== ''
        ...
        <div style={{ marginBottom:24, fontSize:13, color:'#444' }}>
          <b>特記事項：</b>
          <ul style={{ marginLeft:24, marginTop:8, paddingLeft:16 }}>
